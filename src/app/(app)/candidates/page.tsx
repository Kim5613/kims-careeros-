  ...
  const columns: ColumnsType<Candidate> = [
    ...
    { title: '职位', dataIndex: 'position', key: 'position', width: 140 },
    {
      ...
      render: (skills: string[]) => (
        <Space wrap>{skills.map(s => <Tag key={s} color="#8b7cf0" style={{ borderRadius: 8 }}>{s}</Tag>)}</Space>
      ),
    },
    {
      title: '经验(年)', dataIndex: 'experienceYears', key: 'experienceYears', width: 90, align: 'center',
      render: (y: number) => <Tag style={{ borderRadius: 8 }}>{y}年</Tag>,
    },
    {
      title: '来源', dataIndex: 'source', key: 'source', width: 100,
      render: (s: string) => <Tag color="cyan" style={{ borderRadius: 8 }}>{s}</Tag>,
    },
    {
      ...
      render: (status: string, record: Candidate) => (
        <Dropdown
          ...
          trigger={['click']}
        >
          <Tag color={statusConfig[status]?.color} icon={statusConfig[status]?.icon} style={{ cursor: 'pointer', borderRadius: 8 }}>
            {status}
          </Tag>
        </Dropdown>
      ),
    ...
    <div style={{ padding: '40px 48px 24px', background: '#faf8f6', minHeight: '100vh' }}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="总候选人" value={stats.total} prefix={<TeamOutlined />} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="待筛选" value={stats.byStatus['待筛选']} valueStyle={{ color: '#8c8c8c' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="面试中" value={stats.byStatus['面试中']} valueStyle={{ color: '#8b7cf0' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="已录用" value={stats.byStatus['已录用']} valueStyle={{ color: '#52c41a' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="已拒绝" value={stats.byStatus['已拒绝']} valueStyle={{ color: '#ff4d4f' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="暂缓" value={stats.byStatus['暂缓']} valueStyle={{ color: '#faad14' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="放弃" value={stats.byStatus['放弃']} valueStyle={{ color: '#d9d9d9' }} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6} lg={3}>
          <Card size="small" style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Statistic title="人才储备" value={stats.talentPoolCount} prefix={<StarFilled style={{ color: '#faad14' }} />} /></Card>
        </Col>
      </Row>

      {/* Filter Toolbar */}
      <Card style={{ marginBottom: 16, borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}>
        <Space wrap>
          <Input
            placeholder="搜索候选人姓名"
          ...
          <Button type="primary" icon={<PlusOutlined />} style={{ borderRadius: 8 }} onClick={() => setAddModalVisible(true)}>
            新增候选人
          </Button>
        </Space>
      </Card>
      ...
      {/* Candidate Table */}
      <Card style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}>
        <Table
          columns={columns}
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="company" label="当前公司">
                <Input placeholder="请输入当前公司" style={{ borderRadius: 8 }} />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="position" label="当前职位">
                <Input placeholder="请输入当前职位" style={{ borderRadius: 8 }} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="experienceYears" label="工作年限">
                <InputNumber min={0} max={50} placeholder="年" style={{ width: '100%', borderRadius: 8 }} />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name="skills" label="技能标签">
            <Input placeholder="多个技能用逗号分隔，如：Python, TensorFlow, NLP" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="source" label="来源渠道">
            <Select placeholder="请选择来源" style={{ borderRadius: 8 }} options={sourceOptions.map(s => ({ label: s, value: s }))} />
          </Form.Item>
          <Form.Item name="resumeSnapshot" label="简历快照">
            <TextArea rows={3} placeholder="简要描述候选人简历要点" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="notes" label="备注">
            <TextArea rows={2} placeholder="备注信息" style={{ borderRadius: 8 }} />
          </Form.Item>
        </Form>
      </Modal>
        ...
        {selectedCandidate && (
          <div>
            <Row gutter={[16, 16]}>
              ...
              <Col span={12}><Text strong>来源：</Text><Tag color="cyan" style={{ borderRadius: 8 }}>{selectedCandidate.source}</Tag></Col>
              <Col span={12}>
                <Text strong>状态：</Text>
                <Tag color={statusConfig[selectedCandidate.status]?.color} style={{ borderRadius: 8 }}>
                  {selectedCandidate.status}
                </Tag>
              </Col>
              <Col span={24}>
                <Text strong>技能：</Text>
                <Space wrap>{selectedCandidate.skills.map(s => <Tag key={s} color="#8b7cf0" style={{ borderRadius: 8 }}>{s}</Tag>)}</Space>
              </Col>
              <Col span={12}>
      ...
      >
        <Form form={interviewForm} layout="vertical" onFinish={handleAddInterview}>
          <Form.Item name="round" label="面试轮次" rules={[{ required: true, message: '请输入面试轮次' }]}>
            <InputNumber min={1} max={10} placeholder="第几轮" style={{ width: '100%', borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="interviewDate" label="面试日期" rules={[{ required: true, message: '请输入面试日期' }]}>
            <Input placeholder="如：2024-03-20" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="interviewer" label="面试官">
            <Input placeholder="请输入面试官姓名" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="evaluation" label="面试评价">
            <TextArea rows={3} placeholder="请输入面试评价" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="notes" label="备注">
            <TextArea rows={2} placeholder="备注信息" style={{ borderRadius: 8 }} />
          </Form.Item>

  ...
  return (
    <div style={{ padding: '40px 48px 24px', background: '#faf8f6', minHeight: '100vh' }}>
      ...
      {filteredInsights.length === 0 ? (
        <Card style={{ borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}><Empty description="暂无洞察数据" /></Card>
      ) : (
        <Row gutter={[16, 16]}>
          {filteredInsights.map(insight => {
            ...
            return (
              <Col xs={24} sm={24} md={12} lg={8} key={insight.id}>
                <Card
                  hoverable
                  style={{ height: '100%', borderRadius: 8, boxShadow: '0 0 0 1px rgba(0,0,0,0.03), 0 1px 2px rgba(0,0,0,0.02)' }}
                    ...
                    title={
                      <Space direction="vertical" size={0}>
                        ...
                        <Space size={4} style={{ marginTop: 4 }}>
                          <Tag color={config.color} style={{ borderRadius: 8 }}>{insight.category}</Tag>
                          {insight.industry && <Tag style={{ borderRadius: 8 }}>{insight.industry}</Tag>}
                        </Space>
                      </Space>
                    }
                    description={
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleAdd}>
          <Form.Item name="title" label="标题" rules={[{ required: true, message: '请输入标题' }]}>
            <Input placeholder="洞察标题" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="content" label="内容" rules={[{ required: true, message: '请输入内容' }]}>
            <TextArea rows={6} placeholder="洞察详细内容" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Row gutter={16}>
            <Col span={8}>
            ...
            <Col span={8}>
              <Form.Item name="industry" label="行业">
                <Select placeholder="请选择" allowClear style={{ borderRadius: 8 }} options={industryOptions.map(i => ({ label: i, value: i }))} />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item name="position" label="岗位">
                <Input placeholder="相关岗位" style={{ borderRadius: 8 }} />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name="source" label="数据来源">
            <Input placeholder="数据或信息的来源" style={{ borderRadius: 8 }} />
          </Form.Item>
          <Form.Item name="dataPoints" label="数据点 (JSON格式)">
            <TextArea rows={3} placeholder='{"key1":"value1","key2":"value2"}' style={{ borderRadius: 8 }} />
          </Form.Item>
        </Form>
      </Modal>
    </div>

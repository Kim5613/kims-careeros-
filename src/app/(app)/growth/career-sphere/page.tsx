...
// All data from OB project reviews — no hardcoded content
const DOMAINS: DomainNode[] = ALL_DOMAINS.map(function(d) {
  return {
    id: d.id,
    label: d.label,
    href: d.available ? '/growth/domain/' + d.id : undefined,
  };
});
// Ensure we always have at least 5 nodes for the sphere
...
export default function CareerSpherePage() {
  ...
  const handleClick = useCallback((d: DomainNode) => {
    if (d.href) navigateWithTransition(() => router.push('/growth/domain/' + d.id));
  ...
  return (
    <div style={{ background: '#0d0d0d', minHeight: '100vh', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Right: Pure text list on background */}

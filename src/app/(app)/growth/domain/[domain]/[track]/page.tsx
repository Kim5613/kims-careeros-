    ...
    return (
      <div style={{ padding: '80px 32px', background: '#0d0d0d', minHeight: '100vh', display: 'flex', justifyContent: 'center' }}>
        <Result
          ...
          extra={<Button type="primary" style={{ borderRadius: 20 }} href={`/growth/domain/${domainSlug}`}>返回{domain?.label || '上一级'}</Button>}
        />
      </div>
    );
  }
  ...
  return (
    <div style={{ background: '#0d0d0d', minHeight: '100vh' }}>
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '40px 32px' }}>
        {/* Breadcrumb: 职业宇宙 - HR - TA */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32 }}>
          <span
            onClick={() => navigateWithTransition(() => router.push('/growth/career-sphere'))}
          ...
          <span
            onClick={() => navigateWithTransition(() => router.push(`/growth/domain/${domainSlug}`))}
            style={{ color: '#888', fontSize: 14, textDecoration: 'none', letterSpacing: 1, cursor: 'pointer', transition: 'color 0.25s' }}
            onMouseEnter={(e) => { e.currentTarget.style.color = '#ccc'; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = '#888'; }}
          >

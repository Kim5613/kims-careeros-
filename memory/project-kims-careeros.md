...
- 访问：http://139.196.159.68（密码 111）
- 本地 21 条待办/日程已同步到服务器
- 应用本地 PostgreSQL（非云 RDS，节省成本）
- 已修复：standalone 模式导致静态资源加载失败、Cookie Secure 标志导致 HTTP 登录失败

待完成：
- 域名 HTTPS：kimstar.cn（备案中，通过后配置 DNS + certbot）
- 分享链接功能
- JWT_SECRET 改为强随机值

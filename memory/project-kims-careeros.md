...
| 版本 | 日期 | 内容 |
|------|------|------|
| **v1.3** | 2026-07-30 | **封版**：结构化简历+AI标签+5项数据互联+内部战役模板+身份名牌×内部战役联动。详见 PRD §14.7。上线前两轮审查修复 6 严重 bug（见决策记录 2026-07-30） |
| **v1.2.2** | 2026-07-24 | 内部战役+大师团增强+部署健壮性 |
| **v1.2.1** | 2026-07-22 | AI 上线修复：stopWhen/流式协议/env校验/鉴权加固/桌宠持久化/日视图重叠事件分组修复/onError 全路由覆盖 |
| **v1.2** | 2026-07-21 | AI 集成 + UX 重构：求职流水线五阶段、岗位诊断API、面试追踪表单、全站设计统一、侧边栏平级6入口、42项UI修复、设计规范+skill规则 |
...
### 2026-07-30 [质量] v1.3 上线前两轮审查：6 严重 bug 全修，教训沉淀进 TECH-PITFALLS

- **方式**：主审（API/lib 逐文件）+ 两个子代理深审（identity 页 / battle+applications 页）+ 本地构建 + 解析器 node 实测 + 带登录态运行时冒烟
- **严重 bug（全部修复并验证）**：
  1. identity 页「个人信息编辑」Modal 被未闭合 JSX 注释吞掉——语法合法构建通过，功能完全失效
  2. 模板解析器 `escapeRegex` 把调用方正则模式转义成字面量 → 缘由/目标/过程/结果/不足永远解析为空
  3. 解析器 `\s*` 跨行吞噬 → 字段留空时后续值全部错一行
  4. `mapApp` 丢 `resumeId` → 编辑保存静默解除简历关联
  5. 战役 POST 解构出 skills 不落库 → 新建丢全部能力沉淀
  6. useApiList 无竞态守卫 → 快速切换简历旧响应覆盖新数据
- **流程修正**：autoTag 改为仅新建触发（编辑不覆盖手动标签、省 DeepSeek 调用）；PATCH 加字段白名单 + P2025→404；所有本地降级路径加「未同步到服务器」提示
- **基建**：git 历史清除 deploy.zip（108MB→10.55MB）force push；tsconfig.tsbuildinfo 移出跟踪
- **教训**：构建绿 ≠ 功能对。这批 bug 全部是"静默失效"类——不报错、类型检查通过，只有真用才暴露。已沉淀为 TECH-PITFALLS §六（#13-17）+ 验收清单新增 4 项
- commits：`f058cd6`（批一）、`2206fe2`（批二）

### 2026-07-27 [交互] 新增简历弹窗内嵌三板块 + 导出默认 HTML（Kim 反查 v1.3 后补）

- **背景**：v1.3 首版把「创建简历」和「录入经历」拆成两步（先建空简历→下方经历面板补录），Kim 反馈与需求不符——点击「新增简历」后表单本身就要分板块
- **决策**：
  - 创建弹窗改为三板块：简历版本（标题/目标岗位/目标公司/版本号/默认）+ 工作经历 Form.List + 项目经历 Form.List，经历当场无限新增，保存时先建简历再逐条 POST 经历；**经历编辑 = 点击简历卡片弹窗**（两板块同页无 Tab，与新增同格式）
  - 空行自动过滤、半填行报错指明第几条，经历部分失败时 warning 汇总
  - 导出改 Dropdown.Button：主按钮一键导 HTML（默认），箭头菜单选 MD
  - 导出排版对齐 Kim 规格：条目间分隔线改明显实线（#999），MD 空章节补 `---`
  - 简历预览弹窗改为 iframe 渲染导出 HTML（所见即所得）
- **同批修复**：useApiList 查询参数 405 假保存（见技术坑点）、复制简历连带经历、导出含求职意向+补充说明章节、经历 ↑↓ 排序

### 2026-07-27 [交互定稿] 导出弹窗化 + 卡片紧凑化 + 经历编辑弹窗化（Kim 逐轮反馈收敛）

- **导出简历**：从"藏在经历面板里"改为页面右上角主按钮；点击弹出**导出卡片**——左侧 iframe 实时预览、右侧个人信息摘要 + 简历版本单选、底部 HTML（主）/ MD；卡片 📤 与经历弹窗「导出此版本」均打开同一弹窗并预选
- **简历卡片**：3 列 → 紧凑 4 列（lg=6），目标岗位@公司合并单行、去分隔线
- **经历编辑**：页面下方大面板 → 点击卡片弹窗；弹窗内 Tab 切换 → 两板块同页（Kim：填写时什么格式，打开就什么格式）
- **备注字段移除**：表单删除「简历备注（旧版兼容）」，API 解除 content 必填，旧数据导出时仍作「补充说明」
- **Kim 提出的待确认需求**：导出时"选择个人信息的版本"——当前 PersonalInfo 单例，若需多份需改数据模型（已记入 PRD §14.6）
- 本批坑点：useApiList/useApiSingle mockData 内联数组无限重拉（弹窗闪动），见技术坑点

### 2026-07-27 [数据模型] 简历从单一 Markdown 字段升级为结构化模型

- **决策**：新增 3 个 Prisma 模型（PersonalInfo / WorkExperience / ProjectExperience）替代 Resume 的单一 content TextArea，工作/项目经历与 Resume 多对一关联（Cascade 删除）
- **为什么**：
  - 单一 Markdown 无法支撑结构化查询、AI 生成、格式化导出
  - 新版导出要求精确的字段级别排版（公司名左对齐/时间右对齐/编号要点）
  - PersonalInfo 作为单例全版本共享（姓名/电话/邮箱/学校/专业），避免每份简历重复填写
- **技术方案**：
  - PersonalInfo API 使用 GET + POST upsert（单例模式，需新建 useApiSingle hook）
  - 工作/项目经历 API 使用 `?resumeId=` 查询参数筛选（扁平路由，非嵌套路由）
  - 切换简历时通过 React 状态触发 useApiList 自动 refetch，无需 key-based remount
  - 保留 Resume.content 字段作为旧版兼容（编辑 Modal 标注"备注/旧版兼容"）
- **导出**：纯客户端 HTML/MD 生成（`src/lib/export-resume.ts`），不依赖服务端

### 2026-07-23 [架构] 侧边栏重构为「两维度×七模块」

- **决策**：将侧边栏从旧的 6 项平铺结构（首页/求职/职业宇宙/HR工作台/知识库/设置）重构为「人生职业操作系统」定义的两维度×七模块架构
- **为什么**：与个人知识管理框架对齐，统一心智模型。维度一（个人经营系统）：身份铭牌/实战沙盘/认知实验室/身价账本/职业星图；维度二（工作资产系统）：人才弹药库/公司瞭望台
- **实施**：新建 17 个占位页面（含 5 个 redirect 到已有页面），重构 Sidebar.tsx 使用 Ant Design SubMenu 两级导航，折叠模式展示 8 个图标（首页+7模块+设置），展开宽度从 220px 增至 240px
- **未做**：不删除旧路由，不修改数据库/API
...
| 简历与求职信 | ✅ v1.3 | 结构化升级：个人信息单例 + 工作/项目经历无限新增 + HTML/MD 一键导出；创建=三板块弹窗、编辑=点卡片弹窗（无 Tab）、导出=左预览右选择弹窗；交互定稿见 PRD §14.3.1 |
...
**2026-07-24 | [功能] 内部战役模块（v1.2.2 P1）**
- 决策：内部战役定位为项目经历结构化记录池，最终目标是简历数据源 + 职业宇宙能力证明
- 表单三区块：公司信息（关联 companies 表，选择自动带出行业/规模/背景）+ 项目内容（19字段）+ 能力沉淀
- 自研 contentEditable 富文本编辑器（B/U/列表/字号/文字颜色/高亮/全屏编辑），不用第三方库
- 四大分类（BP/晋升/PIP/其他）经用户反馈后取消，改为统一总览网格
- 详情面板和表单均用居中浮窗（1100px × 95vh），非右侧滑出
- 数据模型：battle_projects + battle_project_skills 两张表，Company 表新增 background 字段
- P2 待做：手动打标+AI推荐→职业宇宙联动；P3 待做：简历AI一键生成

## 技术坑点

**2026-07-30 | [静默失效] v1.3 深审揪出 6 个"构建全绿但功能是坏的"的 bug**
- 特征：不报错、类型检查通过、构建通过，只有真用功能才暴露。防御 = 功能写完真点一遍，解析器拿真实输入跑一次
- ① JSX 块注释未闭合（`{/* ════` 忘了收尾 `*/}`）吞掉整个 Modal → 功能完全失效
- ② 正则工具函数内部 escapeRegex，调用方传 `'缘由.*'` 模式 → 解析永远为空
- ③ 行级解析 `\s*` 匹配换行 → 字段留空时后面值全部错一行（行内用 `[ \t]*`、值用 `[^\n]*`）
- ④ mapApp 丢 resumeId → 编辑回写静默解除简历关联（map 函数必须透传全部字段）
- ⑤ POST 解构出 skills 不落库，PATCH 有 → POST/PATCH 行为要对称
- ⑥ useApiList refetch 无序号守卫 → 快速切换简历旧响应覆盖新数据；已有真实数据时失败不能再拿 mock 覆盖
- 全部沉淀至 `docs/TECH-PITFALLS.md` §六 #13-17，验收清单同步新增 4 项

**2026-07-27 | [前端] useApiList/useApiSingle 的 mockData 内联数组引发无限重拉（弹窗闪动）**
- 问题：identity 弹窗化后，点击卡片经历列表"加载不出来、频繁闪动"。日志可见 API 被连续反复请求
- 根因：调用方内联传 `mockData: MOCK.filter(...)`（每次渲染新引用）→ hook 的 `refetch` useCallback 依赖含 mockData → refetch 身份每轮渲染都变 → useEffect 反复触发 → setData 引发渲染 → 新 mockData → 再触发，死循环
- 修复：hook 内用 `mockDataRef` 读最新值，refetch 依赖只留 `endpoint`（字符串，仅在切换简历时变）
- 教训：**React hook 的依赖数组里永远不要放调用方内联生成的对象/数组**；要么 ref 化，要么要求调用方 useMemo。自定义 hook 要防御调用方的引用不稳定

**2026-07-27 | [前端] useApiList 带查询参数的 endpoint 导致 PATCH/DELETE 全部 405 假保存**
- 问题：identity 页面给 useApiList 传 `endpoint: '/api/work-experiences?resumeId=xxx'`，hook 内 `${endpoint}/${id}` 拼出 `/api/work-experiences?resumeId=xxx/<id>` → 路径实为集合路由，PATCH/DELETE 405 → hook 的 Mock 降级 catch 吞掉错误做本地假更新 → **界面显示已改，刷新即丢**
- 根因：endpoint 设计假设不含查询串，v1.3 首次引入 `?resumeId=` 用法破坏假设
- 修复：hook 内统一 `const baseEndpoint = endpoint.split('?')[0]`，create/update/remove 全走 baseEndpoint（GET 保留完整 endpoint 用于筛选）
- 教训：**凡 hook 拼接 URL，必须考虑 endpoint 可能带查询串**；Mock 降级静默吞错会让"假成功"难以察觉——数据写操作出问题时应优先怀疑降级路径
